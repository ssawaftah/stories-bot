...
function getUserKeyboard() {
  ...
  // ثم الأزرار المخصصة (كنص عادي، لأن keyboard لا يدعم URL)
...
function getWelcomeForUser(isNewUser, isPending, isVerified, isRejected) {
  ...
  if (isVerified || !data.verification.enabled) {
    return {
      text: data.texts.welcomeRegistered || "📦 <b>مرحباً بعودتك!</b>",
      mediaType: null,
      mediaFileId: null,
      buttons: null,
  ...
  return {
    text: data.texts.welcomeNew || "🎉 <b>مرحباً بك في البوت!</b>\n\nيمكنك استخدام البوت للوصول إلى المحتوى.",
...
async function handleUpdate(update, env) {
  ...
  if (update.message) {
    ...
    if (customBtn) {
      ...
      await sendMessage(chatId, `🔗 <a href="${customBtn.url}">${customBtn.label}</a>`, token, {
        parse_mode: "HTML",
        disable_web_page_preview: true,
        reply_markup: getUserKeyboard(), // إبقاء الأزرار
      });
      return;
    }
