...
function buildBasePayload(chatId, extra) {
  const payload = {
    chat_id: chatId,
    parse_mode: "HTML",
    ...(data.protection.enabled && !extra?.bypass_protection ? { protect_content: true } : {}),
  };
  if (extra) {
...
async function editMessageCaption(chatId, msgId, caption, token, extra) {
  const payload = { chat_id: chatId, message_id: msgId, caption, parse_mode: "HTML" };
  if (extra) Object.assign(payload, extra);
...
function setLogAllActions(enabled) {
  logAllActions = enabled;
}
...
function getWelcomeForUser(isNewUser, isPending, isVerified, isRejected) {
  ...
  if (isVerified || !data.verification.enabled) return {
    text: w.registeredMessage || "📦 <b>مرحباً بعودتك!</b>",
    mediaType: null, mediaFileId: null, buttons: null, userKeyboard: getUserKeyboard(),
  };
  return {
    text: w.text,
...
function buttonsMenu() {
  ...
  if (count > 0) {
    listText = items.map((b, i) => `${i+1}. <b>${b.label}</b> — <code>${b.url}</code>`).join("\n");
  }
  return {
...
function generateButtonId() {
  return 'btn_' + Date.now().toString(36);
}
...
async function handleUpdate(update, env) {
  ...
  if (update.callback_query) {
    ...
    // التحقق من صلاحية المستخدم غير الأدمن
...
async function handleAdminInput(userId, chatId, msg, token, env) {
  ...
  // ===== تعديل نصوص مختلفة =====
  const textEdits = {
    edit_welcome: { field: () => (data.welcome.text = text), menu: welcomeMenu, key: "admin_welcome" },
    edit_registered: { field: () => (data.welcome.registeredMessage = text), menu: welcomeMenu, key: "admin_welcome" },
    edit_stop: { field: () => (data.settings.stopMessage = text), menu: botSettingsMenu, key: null },
  };
  ...
  if (state.action === "btn_add_label" && state.step === "waiting_url") {
    ...
    const newBtn = { id: generateButtonId(), label, url };
    data.buttons.items.push(newBtn);
    await saveData(env);
  ...
  if (state.action === "btn_edit_select" && state.step === "waiting_id") {
    ...
    clearAdminState(userId); // سنتعامل مع التعديل عبر الكولباك
    return true;
  }
  ...
  // ===== باقي إدخالات الأدمن (حظر، بث، أوامر، إلخ) =====
  // (نفس الكود السابق — تم حذف التفاصيل للاختصار)
  // ... 
  return false;
}
...
async function handleAdminCallback(userId, cbData, chatId, msgId, token, env) {
  ...
  if (cbData === "btn_edit_field_label" || cbData === "btn_edit_field_url") {
    ...
    const idx = state.temp?.editIndex;
    if (idx === undefined) {
      await sendMessage(chatId, "⚠️ حدث خطأ. حاول مرة أخرى.", token);
      clearAdminState(userId);
      return;
    }
    const items = data.buttons?.items || [];
  ...
  if (cbData === "users_delete") {
    ...
    const buttons = users.slice(0, 15).map((uid) => [
      { text: `🗑️ ${data.users[uid]?.name || uid}`, callback_data: "users_delete_confirm_" + uid },
    ]);
    buttons.push([{ text: "🔙 رجوع", callback_data: "admin_users" }]);
    await editMessage(chatId, msgId, "🗑️ <b>اختر مستخدماً للحذف:</b>", token, { reply_markup: { inline_keyboard: buttons } });
    return;
  ...
  if (cbData === "fs_edit") {
    ...
    const buttons = fs.list.map((s, i) => [
      { text: `✏️ ${s.name || s.type + " " + (i + 1)}`, callback_data: "fs_edit_select_" + i },
    ]);
    buttons.push([{ text: "🔙 رجوع", callback_data: "admin_forced_subscription" }]);
    await editMessage(chatId, msgId, "✏️ <b>اختر اشتراكاً لتعديله:</b>", token, { reply_markup: { inline_keyboard: buttons } });
    return;
  ...
  if (cbData === "fs_delete") {
    ...
    const buttons = fs.list.map((s, i) => [
      { text: `🗑️ ${s.name || s.type + " " + (i + 1)}`, callback_data: "fs_delete_confirm_" + i },
    ]);
    buttons.push([{ text: "🔙 رجوع", callback_data: "admin_forced_subscription" }]);
    await editMessage(chatId, msgId, "🗑️ <b>اختر اشتراكاً للحذف:</b>", token, { reply_markup: { inline_keyboard: buttons } });
    return;
  ...
  if (cbData === "btn_edit") {
    const items = data.buttons?.items || [];
    if (items.length === 0) {
      await sendMessage(chatId, "⚠️ لا توجد أزرار لتعديلها.", token);
      return;
    }
    state.action = "btn_edit_select";
    state.step = "waiting_id";
